...
BASE_GNN_ADJACENCY_MODES = {"static", "prior", "adaptive"}
BASE_GNN_TEMPORAL_OPERATORS = {"conv", "mpnn"}
BASE_GNN_OPERATOR_ALIASES = {
    "dynamic_rel_conv": "prior_conv",
    "dynamic_edge_mpnn": "prior_mpnn",
}


def normalize_base_gnn_adjacency_mode(value: Any, default: str = "prior") -> str:
    mode = str(first_not_none(value, default)).strip().lower().replace("-", "_")
    if mode not in BASE_GNN_ADJACENCY_MODES:
        raise ValueError(
            f"Unsupported base_gnn adjacency mode: {value}. "
            f"Expected one of: {sorted(BASE_GNN_ADJACENCY_MODES)}"
        )
    return mode


def normalize_base_gnn_temporal_operator(value: Any, default: str = "conv") -> str:
    mode = str(first_not_none(value, default)).strip().lower().replace("-", "_")
    if mode not in BASE_GNN_TEMPORAL_OPERATORS:
        raise ValueError(
            f"Unsupported base_gnn temporal operator: {value}. "
            f"Expected one of: {sorted(BASE_GNN_TEMPORAL_OPERATORS)}"
        )
    return mode


def canonicalize_base_gnn_operator_name(
    operator_name: Any,
    default_adj_mode: str = "prior",
    default_temporal_operator: str = "conv",
) -> str:
    raw = str(first_not_none(operator_name, "")).strip().lower().replace("-", "_")
    raw = BASE_GNN_OPERATOR_ALIASES.get(raw, raw)

    default_adj_mode = normalize_base_gnn_adjacency_mode(default_adj_mode, default="prior")
    default_temporal_operator = normalize_base_gnn_temporal_operator(default_temporal_operator, default="conv")

    if not raw:
        return f"{default_adj_mode}_{default_temporal_operator}"

    if raw in BASE_GNN_TEMPORAL_OPERATORS:
        return f"{default_adj_mode}_{raw}"

    if raw in BASE_GNN_ADJACENCY_MODES:
    ...
    for suffix, temporal_mode in suffix_map.items():
        ...
        if raw.endswith(token):
            ...
            adjacency_mode = normalize_base_gnn_adjacency_mode(adjacency_mode, default=default_adj_mode)
            return f"{adjacency_mode}_{temporal_mode}"

    raise ValueError(
        f"Unsupported base_gnn graph_operator: {operator_name}. "
        "Use forms like static_conv, prior_conv, adaptive_conv, static_mpnn, prior_mpnn, adaptive_mpnn."
    )


def split_base_gnn_operator_name(
    operator_name: Any,
    default_adj_mode: str = "prior",
    default_temporal_operator: str = "conv",
) -> Tuple[str, str, str]:
    canonical_name = canonicalize_base_gnn_operator_name(
...
def normalize_operator_candidate_list(
...
def add_cli_override_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--base-gnn-default-adj-mode", type=str, default=None)
    parser.add_argument("--base-gnn-default-temporal-operator", type=str, default=None)
    parser.add_argument("--base-gnn-relation-fusion-mode", type=str, default=None)
    parser.add_argument("--base-gnn-prior-strength", type=float, default=None)
    parser.add_argument("--base-gnn-adaptive-rank", type=int, default=None)
    parser.add_argument("--base-gnn-adaptive-scale", type=float, default=None)
...
def apply_cli_overrides(cfg: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    scalar_overrides = {
        ...
        "base_gnn_default_adj_mode": args.base_gnn_default_adj_mode,
        "base_gnn_default_temporal_operator": args.base_gnn_default_temporal_operator,
        "base_gnn_relation_fusion_mode": args.base_gnn_relation_fusion_mode,
        "base_gnn_prior_strength": args.base_gnn_prior_strength,
        "base_gnn_adaptive_rank": args.base_gnn_adaptive_rank,
        "base_gnn_adaptive_scale": args.base_gnn_adaptive_scale,
...
def resolve_extended_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    base_gnn_cfg = deep_merge_dicts(base_gnn_defaults, get_nested_value(resolved, "base_gnn", {}))
    ...
    resolved["base_gnn_default_adj_mode"] = normalize_base_gnn_adjacency_mode(
        first_not_none(resolved.get("base_gnn_default_adj_mode"), base_gnn_cfg.get("default_adj_mode"), "prior"),
        default="prior",
    )
    resolved["base_gnn_default_temporal_operator"] = normalize_base_gnn_temporal_operator(
        first_not_none(resolved.get("base_gnn_default_temporal_operator"), base_gnn_cfg.get("default_temporal_operator"), "conv"),
        default="conv",
    )
    resolved["base_gnn_relation_fusion_mode"] = str(
        first_not_none(resolved.get("base_gnn_relation_fusion_mode"), base_gnn_cfg.get("relation_fusion_mode"), "attention")
    ).strip().lower().replace("-", "_")
    resolved["base_gnn_prior_strength"] = float(
        first_not_none(resolved.get("base_gnn_prior_strength"), base_gnn_cfg.get("prior_strength"), 1.0)
    )
    resolved["base_gnn_adaptive_rank"] = int(
        first_not_none(resolved.get("base_gnn_adaptive_rank"), base_gnn_cfg.get("adaptive_rank"), 16)
    )
    resolved["base_gnn_adaptive_scale"] = float(
        first_not_none(resolved.get("base_gnn_adaptive_scale"), base_gnn_cfg.get("adaptive_scale"), 1.0)
    )

    if resolved["base_gnn_relation_fusion_mode"] not in {"attention", "mean"}:
        raise ValueError(
            "Unsupported base_gnn_relation_fusion_mode: "
            f"{resolved['base_gnn_relation_fusion_mode']}. Expected one of: attention, mean"
        )
    if resolved["base_gnn_prior_strength"] <= 0.0:
        raise ValueError("base_gnn_prior_strength must be > 0")
    if resolved["base_gnn_adaptive_rank"] <= 0:
        raise ValueError("base_gnn_adaptive_rank must be > 0")
    if resolved["base_gnn_adaptive_scale"] <= 0.0:
        raise ValueError("base_gnn_adaptive_scale must be > 0")

    default_graph_operator = (
        f"{resolved['base_gnn_default_adj_mode']}_{resolved['base_gnn_default_temporal_operator']}"
    )
    resolved["graph_operator"] = canonicalize_base_gnn_operator_name(
        first_not_none(resolved.get("graph_operator"), base_gnn_cfg.get("graph_operator"), default_graph_operator),
        default_adj_mode=resolved["base_gnn_default_adj_mode"],
        default_temporal_operator=resolved["base_gnn_default_temporal_operator"],
    )
    raw_operator_candidates = first_not_none(
        resolved.get("operator_candidates"),
        base_gnn_cfg.get("operator_candidates"),
        base_gnn_defaults["operator_candidates"],
    )
    ...
    if raw_operator_candidates_list and all(
        str(x).strip().lower().replace("-", "_") in BASE_GNN_OPERATOR_ALIASES
        for x in raw_operator_candidates_list
    ):
        raw_operator_candidates_list = list(base_gnn_cfg.get("operator_candidates") or base_gnn_defaults["operator_candidates"])
    resolved["operator_candidates"] = normalize_operator_candidate_list(
        raw_operator_candidates_list or base_gnn_cfg.get("operator_candidates") or base_gnn_defaults["operator_candidates"],
        default_adj_mode=resolved["base_gnn_default_adj_mode"],
        default_temporal_operator=resolved["base_gnn_default_temporal_operator"],
    )
    if not resolved["operator_candidates"]:
        raise ValueError("operator_candidates must contain at least one base_gnn operator")
    ...
    resolved["base_gnn"] = {
        "default_adj_mode": resolved["base_gnn_default_adj_mode"],
        "default_temporal_operator": resolved["base_gnn_default_temporal_operator"],
        "relation_fusion_mode": resolved["base_gnn_relation_fusion_mode"],
        "prior_strength": resolved["base_gnn_prior_strength"],
        "adaptive_rank": resolved["base_gnn_adaptive_rank"],
        "adaptive_scale": resolved["base_gnn_adaptive_scale"],
...
class BaseGraphAdjacencyFactory(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.n_nodes = int(n_nodes)
        self.adjacency_mode = normalize_base_gnn_adjacency_mode(adjacency_mode, default="prior")
...
class SingleGraphOperatorBlock(nn.Module):
    def __init__(
        ...
        super().__init__()
        canonical_name, adjacency_mode, temporal_operator = split_base_gnn_operator_name(
            operator_name=operator_name,
            default_adj_mode=default_adj_mode,
            default_temporal_operator=default_temporal_operator,
        )
    ...
    def forward(
        ...
        latest_edge_state = edge_seq[:, -1, :, :]
        adjacency_bundle = self.adjacency_factory(latest_edge_state)
        flat_adjacency = adjacency_bundle["selected"].unsqueeze(1).expand(-1, seq_len, -1).reshape(bsz * seq_len, n_edges)

        out = node_seq.reshape(bsz * seq_len, n_nodes, node_dim)
        flat_edges = edge_seq.reshape(bsz * seq_len, n_edges, edge_dim)

        for layer in self.layers:
            out = layer(out, flat_edges, flat_adjacency)
...
class BaseTemporalFusionModel(nn.Module):
    def __init__(
        self,
        node_dim: int,
        edge_dim: int,
        ...
        default_adj_mode = normalize_base_gnn_adjacency_mode(cfg.get("base_gnn_default_adj_mode"), default="prior")
        default_temporal_operator = normalize_base_gnn_temporal_operator(
            cfg.get("base_gnn_default_temporal_operator"),
            default="conv",
        )
        ...
        self.edge_relation_fusion = EdgeRelationFusion(
            hidden_dim=edge_hidden_dim,
            num_relations=len(self.relation_names),
            fusion_hidden_dim=int(cfg["fusion_hidden_dim"]),
            mode=str(first_not_none(cfg.get("base_gnn_relation_fusion_mode"), "attention")),
        )
        self.graph_block = SingleGraphOperatorBlock(
            ...
            dropout=dropout,
            prior_strength=float(first_not_none(cfg.get("base_gnn_prior_strength"), 1.0)),
            adaptive_rank=int(first_not_none(cfg.get("base_gnn_adaptive_rank"), 16)),
            adaptive_scale=float(first_not_none(cfg.get("base_gnn_adaptive_scale"), 1.0)),
            default_adj_mode=default_adj_mode,
            default_temporal_operator=default_temporal_operator,
        )
    ...
    def forward(
        ...
        fused_edge_seq, relation_weights = self.edge_relation_fusion(edge_seq)
        ...
        if return_aux:
            graph_node_seq, graph_reg, graph_aux = graph_block_out
        else:
            graph_node_seq, graph_reg = graph_block_out
            graph_aux = {}
        readout_seq, readout_aux = self.readout(graph_node_seq, self.target_node)
        ...
        if return_aux:
            ...
            outputs["relation_node_seq"] = graph_node_seq.unsqueeze(2)
            outputs["fused_node_seq"] = graph_node_seq
            outputs["fused_edge_seq"] = fused_edge_seq
...
MultigraphTemporalFusionModel = BaseTemporalFusionModel
...
def build_model_runtime_cfg(base_cfg: Dict[str, Any], operator_name: str, is_ablation_context: bool) -> Dict[str, Any]:
    ...
    default_adj_mode = normalize_base_gnn_adjacency_mode(run_cfg.get("base_gnn_default_adj_mode"), default="prior")
    default_temporal_operator = normalize_base_gnn_temporal_operator(
        run_cfg.get("base_gnn_default_temporal_operator"),
        default="conv",
    )
    run_cfg["graph_operator"] = canonicalize_base_gnn_operator_name(
        operator_name,
        default_adj_mode=default_adj_mode,
        default_temporal_operator=default_temporal_operator,
    )
...
def build_model_for_cfg(cfg: Dict[str, Any]) -> BaseTemporalFusionModel:
    model = BaseTemporalFusionModel(
