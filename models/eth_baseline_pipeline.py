...
SUPPORTED_BASELINE_MODELS = {"linear_regression", "temporal_conv"}
BASELINE_MODEL_ALIASES = {
    ...
    "temporal_nn": "temporal_conv",
    "temporal_mlp": "temporal_conv",
    "temporal_cnn": "temporal_conv",
...
def canonicalize_model_name(name: Any, default: str = "linear_regression") -> str:
    raw = str(name or "").strip().lower().replace("-", "_")
    if not raw:
        return default
    canonical = BASELINE_MODEL_ALIASES.get(raw, raw)
    if canonical not in SUPPORTED_BASELINE_MODELS:
        supported = ", ".join(sorted(SUPPORTED_BASELINE_MODELS))
        raise ValueError(f"Unsupported ETH baseline model: {name}. Expected one of: {supported}")
    return canonical


def normalize_model_candidate_list(candidates: Any) -> List[str]:
    if candidates is None:
        raw_items = ["linear_regression", "temporal_conv"]
    elif isinstance(candidates, str):
        raw_items = [x.strip() for x in candidates.split(",") if x.strip()]
    else:
        raw_items = [str(x).strip() for x in candidates if str(x).strip()]

    normalized: List[str] = []
    seen: set = set()
    for item in raw_items:
        canonical = canonicalize_model_name(item)
        if canonical in seen:
            continue
        seen.add(canonical)
        normalized.append(canonical)
    return normalized or ["linear_regression", "temporal_conv"]
...
def resolve_runtime_overrides(cfg: Dict[str, Any], config_path: Path) -> Dict[str, Any]:
    ...
    baseline_cfg = get_nested_value(resolved, "eth_baseline", {}) or {}
    ...
    artifact_root_base, artifact_root = build_artifact_root_paths(
        ...
        or baseline_cfg.get("artifact_root")
...
def add_cli_override_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--book-levels", type=int, default=None)
    parser.add_argument("--top-levels", type=int, default=None)
    parser.add_argument("--near-levels", type=int, default=None)
    ...
    parser.add_argument("--eth-baseline-hidden-dim", type=int, default=None)
    parser.add_argument("--eth-baseline-temporal-layers", type=int, default=None)
    parser.add_argument("--eth-baseline-temporal-kernel-size", type=int, default=None)
    parser.add_argument("--eth-baseline-dropout", type=float, default=None)
...
def apply_cli_overrides(cfg: Dict[str, Any], args: argparse.Namespace) -> Dict[str, Any]:
    ...
    scalar_overrides = {
        ...
        "eth_baseline_hidden_dim": args.eth_baseline_hidden_dim,
        "eth_baseline_temporal_layers": args.eth_baseline_temporal_layers,
        "eth_baseline_temporal_kernel_size": args.eth_baseline_temporal_kernel_size,
        "eth_baseline_dropout": args.eth_baseline_dropout,
...
def resolve_extended_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    baseline_defaults = {
        "target_asset": "ETH",
        "default_model": "temporal_conv",
        "operator_candidates": ["linear_regression", "temporal_conv"],
        "run_full_operator_ablation": True,
        "hidden_dim": 48,
        "temporal_layers": 3,
        "temporal_kernel_size": 3,
        "dropout": float(resolved.get("dropout", 0.15) or 0.15),
    }

    resolved["label_mode"] = str(first_not_none(resolved.get("label_mode"), "fixed_horizon"))
    resolved["objective_mode"] = str(first_not_none(resolved.get("objective_mode"), "execution_aware"))
    ...
    baseline_cfg = deep_merge_dicts(baseline_defaults, get_nested_value(resolved, "eth_baseline", {}))

    resolved["target_asset"] = str(first_not_none(baseline_cfg.get("target_asset"), resolved.get("target_asset"), "ETH")).strip().upper()
    resolved["assets"] = [resolved["target_asset"]]
    ...
    resolved["threshold_search_metric"] = str(first_not_none(resolved.get("threshold_search_metric"), backtest_cfg.get("threshold_search_metric"), "composite"))
    resolved["allow_timeout_trades"] = parse_bool(first_not_none(resolved.get("allow_timeout_trades"), backtest_cfg.get("allow_timeout_trades"), True), True)
    default_exit_mode = "realized_event" if str(resolved["label_mode"]) == "triple_barrier" else "fixed_horizon"
    resolved["backtest_exit_mode"] = str(first_not_none(resolved.get("backtest_exit_mode"), backtest_cfg.get("exit_mode"), default_exit_mode))

    resolved["eth_baseline_hidden_dim"] = int(first_not_none(resolved.get("eth_baseline_hidden_dim"), baseline_cfg.get("hidden_dim"), 48))
    resolved["eth_baseline_temporal_layers"] = int(first_not_none(resolved.get("eth_baseline_temporal_layers"), baseline_cfg.get("temporal_layers"), 3))
    resolved["eth_baseline_temporal_kernel_size"] = int(first_not_none(resolved.get("eth_baseline_temporal_kernel_size"), baseline_cfg.get("temporal_kernel_size"), 3))
    resolved["eth_baseline_dropout"] = float(first_not_none(resolved.get("eth_baseline_dropout"), baseline_cfg.get("dropout"), resolved.get("dropout"), 0.15))
    resolved["graph_operator"] = canonicalize_model_name(
        first_not_none(
            baseline_cfg.get("default_model"),
            resolved.get("graph_operator"),
            "temporal_conv",
        ),
        default="temporal_conv",
    )
    resolved["operator_candidates"] = normalize_model_candidate_list(
        first_not_none(baseline_cfg.get("operator_candidates"), resolved.get("operator_candidates"), ["linear_regression", "temporal_conv"])
    )
    resolved["run_full_operator_ablation"] = parse_bool(
        first_not_none(baseline_cfg.get("run_full_operator_ablation"), resolved.get("run_full_operator_ablation"), True),
        True,
    )
    ...
    if resolved["eth_baseline_hidden_dim"] <= 0:
        raise ValueError("eth_baseline_hidden_dim must be > 0")
    if resolved["eth_baseline_temporal_layers"] <= 0:
        raise ValueError("eth_baseline_temporal_layers must be > 0")
    if resolved["eth_baseline_temporal_kernel_size"] <= 1:
        raise ValueError("eth_baseline_temporal_kernel_size must be > 1")
    if not (0.0 <= resolved["eth_baseline_dropout"] < 1.0):
        raise ValueError("eth_baseline_dropout must be in [0, 1)")
    ...
    resolved["eth_baseline"] = {
        "artifact_root": str(first_not_none(baseline_cfg.get("artifact_root"), resolved.get("artifact_root"))),
        "target_asset": resolved["target_asset"],
        "default_model": resolved["graph_operator"],
        "operator_candidates": list(resolved["operator_candidates"]),
        "run_full_operator_ablation": resolved["run_full_operator_ablation"],
        "hidden_dim": resolved["eth_baseline_hidden_dim"],
        "temporal_layers": resolved["eth_baseline_temporal_layers"],
        "temporal_kernel_size": resolved["eth_baseline_temporal_kernel_size"],
        "dropout": resolved["eth_baseline_dropout"],
    }
    return resolved


def initialize_runtime_globals(cfg: Dict[str, Any]) -> None:
    global CFG, ASSETS, TARGET_ASSET
    global ARTIFACT_ROOT, ARTIFACT_BASE_ROOT, RUN_ID, CONFIG_PATH, DEVICE
    global FREQ, HORIZON_MINUTES, HORIZON_BARS, LOOKBACK_BARS, PURGE_GAP_BARS, EXPECTED_DELTA
    ...
    LOGGER.info("TARGET_ASSET=%s", TARGET_ASSET)
    LOGGER.info("ARTIFACT_ROOT=%s", ARTIFACT_ROOT.resolve())
    LOGGER.info(
        "FREQ=%s | HORIZON_BARS=%s | LOOKBACK_BARS=%s | PURGE_GAP_BARS=%s",
        FREQ,
        HORIZON_BARS,
        LOOKBACK_BARS,
        PURGE_GAP_BARS,
...
def finalize_environment_metadata(metadata: Dict[str, Any], run_end_time_utc: str, total_runtime_seconds: float) -> Dict[str, Any]:
...
def send_email_report(subject: str, html_body: str, to_email: str, attachments: List[Path], smtp_settings: Dict[str, Any]) -> None:
...
def horizon_bars_from_clock_minutes(freq: str, horizon_minutes: int = 5) -> int:
    ...
    if horizon_seconds % bar_seconds != 0:
        raise ValueError(f"Horizon of {horizon_minutes} minutes is not an integer number of bars for freq={freq}")
...
def choose_existing_column(df_: pd.DataFrame, candidates: List[str], what: str) -> str:
...
def infer_timestamp_column(df_: pd.DataFrame) -> str:
    return choose_existing_column(df_, ["system_time", "timestamp", "time", "datetime"], "timestamp")


def infer_book_column(df_: pd.DataFrame, side_prefix: str, level: int) -> str:
    candidates = [
        f"{side_prefix}_notional_{level}",
        f"{side_prefix}_vol_{level}",
        f"{side_prefix}_{level}",
    ]
    return choose_existing_column(df_, candidates, f"{side_prefix} level {level}")
    ...
    *,
    ...
    if bad_mask.any():
        ...
        if len(missing_index) == 0:
            raise ValueError(
                f"{name}: irregular index detected but missing timestamps could not be inferred. Examples: {irregular_examples}"
            )
    ...
    if (repaired_diffs != expected_delta).any():
        raise RuntimeError(f"{name}: failed to regularize time index. Expected all deltas to be {expected_delta}.")
...
def load_and_prepare_asset(cfg: Dict[str, Any]) -> pd.DataFrame:
    aligned = load_one_asset_raw(TARGET_ASSET, cfg)
    aligned = aligned.sort_index()
    aligned = aligned[~aligned.index.duplicated(keep="last")].copy()
    aligned = validate_regular_time_index(aligned, EXPECTED_DELTA, name="aligned eth index", fill_missing=True)

    log_mid = np.log(aligned[f"mid_{TARGET_ASSET}"].astype(float).to_numpy() + EPS)
    lr = np.zeros(len(aligned), dtype=np.float64)
    lr[1:] = np.diff(log_mid)
    aligned[f"lr_{TARGET_ASSET}"] = lr.astype(np.float32)
...
def build_target_node_features(df_: pd.DataFrame, cfg: Dict[str, Any]) -> Tuple[np.ndarray, List[str]]:
    book_levels = int(cfg["book_levels"])
    ...
    bids = np.stack([df_[f"bids_notional_{asset}_{i}"].to_numpy(dtype=np.float32) for i in range(book_levels)], axis=1)
    asks = np.stack([df_[f"asks_notional_{asset}_{i}"].to_numpy(dtype=np.float32) for i in range(book_levels)], axis=1)
    ...
    return x_node[:, None, :], feature_names
...
def compute_trade_edge_threshold_log_return(cfg: Dict[str, Any]) -> float:
    ...
    if parse_bool(cfg.get("use_cost_in_label"), True):
        threshold += float(cfg.get("execution_cost_multiplier", 1.0)) * round_trip_cost_as_log_return(float(cfg.get("cost_bps_per_side", 0.0)))
...
def compute_triple_barrier_bps(mid: np.ndarray, cfg: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if pt_sl_mode == "fixed":
        return np.full(n, default_up, dtype=np.float64), np.full(n, default_down, dtype=np.float64)
    ...
    vol = pd.Series(lr_1bar).rolling(window=lookback, min_periods=max(3, lookback // 3)).std().to_numpy(dtype=np.float64)
...
def temporal_collate(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    keys = batch[0].keys()
...
def fit_robust_scaler_train_only_3d(
    ...
    scaler = RobustScaler(with_centering=True, with_scaling=True, quantile_range=(float(q_low), float(q_high)))
...
def initialize_tensor_state(cfg: Dict[str, Any]) -> None:
    global df, TIMESTAMPS, X_NODE_RAW, NODE_FEATURE_NAMES, TARGET_MID, TARGET_LR_1BAR
    ...
    df = load_and_prepare_asset(cfg)
    TIMESTAMPS = pd.to_datetime(df["timestamp"], utc=True)
    X_NODE_RAW, NODE_FEATURE_NAMES = build_target_node_features(df, cfg)
    ...
    if LAST_VALID_T < FIRST_VALID_T:
        raise RuntimeError(f"Not enough rows after slicing. Need lookback={LOOKBACK_BARS} and horizon={HORIZON_BARS}.")
    ...
    LOGGER.info("X_NODE_RAW shape=%s", X_NODE_RAW.shape)
    LOGGER.info(
...
class CausalConv1dBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, dilation: int, dropout: float):
        ...
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size, dilation=dilation, padding=self.pad)
...
class LinearRegressionBaselineModel(nn.Module):
    def __init__(self, feature_dim: int, lookback_bars: int, n_exit_types: int) -> None:
        super().__init__()
        flat_dim = int(feature_dim) * int(lookback_bars)
        self.trade_head = nn.Linear(flat_dim, 1)
        self.dir_head = nn.Linear(flat_dim, 1)
        self.ret_head = nn.Linear(flat_dim, 1)
        self.exit_head = nn.Linear(flat_dim, int(n_exit_types))
        self.tte_head = nn.Linear(flat_dim, 1)

    def forward(self, x_node_seq: torch.Tensor, return_aux: bool = False) -> Dict[str, torch.Tensor]:
        x_target = x_node_seq[:, :, 0, :].reshape(x_node_seq.size(0), -1)
        zero = x_target.new_zeros(())
        return {
            "trade_logit": self.trade_head(x_target).squeeze(-1),
            "dir_logit": self.dir_head(x_target).squeeze(-1),
            "return_pred": self.ret_head(x_target).squeeze(-1),
            "exit_type_logit": self.exit_head(x_target),
            "tte_pred": F.softplus(self.tte_head(x_target).squeeze(-1)) + 1.0,
            "adj_l1": zero,
            "adj_prior": zero,
        }


class TemporalConvBaselineModel(nn.Module):
    def __init__(self, feature_dim: int, lookback_bars: int, hidden_dim: int, num_layers: int, kernel_size: int, dropout: float, n_exit_types: int) -> None:
        super().__init__()
        self.lookback_bars = int(lookback_bars)
        self.input_proj = nn.Linear(int(feature_dim), int(hidden_dim))
        self.time_emb = nn.Parameter(torch.zeros(self.lookback_bars, int(hidden_dim)))
        ...
        trunk_dim = int(hidden_dim) * 2
        self.readout = nn.Sequential(
            nn.LayerNorm(trunk_dim),
            nn.Linear(trunk_dim, int(hidden_dim)),
            nn.GELU(),
            nn.Dropout(float(dropout)),
        )
        self.trade_head = nn.Linear(int(hidden_dim), 1)
        self.dir_head = nn.Linear(int(hidden_dim), 1)
        self.ret_head = nn.Linear(int(hidden_dim), 1)
        self.exit_head = nn.Linear(int(hidden_dim), int(n_exit_types))
        self.tte_head = nn.Linear(int(hidden_dim), 1)

    def forward(self, x_node_seq: torch.Tensor, return_aux: bool = False) -> Dict[str, torch.Tensor]:
        x_target = x_node_seq[:, :, 0, :]
        h = self.input_proj(x_target)
        h = h + self.time_emb.unsqueeze(0)
        h = h.transpose(1, 2).contiguous()
        for layer in self.layers:
            h = layer(h)
        h_time = h.transpose(1, 2).contiguous()
        last_state = h_time[:, -1, :]
        mean_state = h_time.mean(dim=1)
        trunk = self.readout(torch.cat([last_state, mean_state], dim=-1))
        zero = trunk.new_zeros(())
        return {
            "trade_logit": self.trade_head(trunk).squeeze(-1),
            "dir_logit": self.dir_head(trunk).squeeze(-1),
            "return_pred": self.ret_head(trunk).squeeze(-1),
            "exit_type_logit": self.exit_head(trunk),
            "tte_pred": F.softplus(self.tte_head(trunk).squeeze(-1)) + 1.0,
            "adj_l1": zero,
            "adj_prior": zero,
        }


def build_model_for_cfg(cfg: Dict[str, Any]) -> nn.Module:
    operator_name = canonicalize_model_name(cfg.get("graph_operator"))
    feature_dim = int(X_NODE_RAW.shape[-1])
    n_exit_types = len(EXIT_TYPE_NAMES)

    if operator_name == "linear_regression":
        model = LinearRegressionBaselineModel(feature_dim=feature_dim, lookback_bars=LOOKBACK_BARS, n_exit_types=n_exit_types)
    else:
        model = TemporalConvBaselineModel(
            feature_dim=feature_dim,
            lookback_bars=LOOKBACK_BARS,
            hidden_dim=int(cfg["eth_baseline_hidden_dim"]),
            num_layers=int(cfg["eth_baseline_temporal_layers"]),
            kernel_size=int(cfg["eth_baseline_temporal_kernel_size"]),
            dropout=float(cfg["eth_baseline_dropout"]),
            n_exit_types=n_exit_types,
        )
...
def masked_binary_cross_entropy_with_logits(logits: torch.Tensor, targets: torch.Tensor, mask: torch.Tensor, pos_weight: torch.Tensor) -> torch.Tensor:
    valid = mask > 0.5
    if not valid.any():
...
def compute_standard_multitask_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor], loss_state: LossState, cfg: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    adj_reg = float(cfg["adj_l1_lambda"]) * outputs["adj_l1"] + float(cfg["adj_prior_lambda"]) * outputs["adj_prior"]
    total_loss = (
...
def compute_execution_aware_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor], loss_state: LossState, cfg: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    soft_position = compute_soft_position_torch(trade_logit=trade_logit, dir_logit=dir_logit, k=float(cfg["utility_tanh_k"]))
    exec_cost = float(cfg.get("execution_cost_multiplier", 1.0)) * round_trip_cost_as_log_return(float(cfg["cost_bps_per_side"]))
...
def compute_total_loss(outputs: Dict[str, torch.Tensor], batch: Dict[str, torch.Tensor], loss_state: LossState, cfg: Dict[str, Any]) -> Dict[str, torch.Tensor]:
...
def apply_threshold_pair(trade_prob: np.ndarray, dir_prob: np.ndarray, thr_trade: float, thr_dir: float) -> Tuple[np.ndarray, np.ndarray]:
...
def search_best_threshold_pair(
    ...
    for thr_trade in cfg["thr_trade_grid"]:
        for thr_dir in cfg["thr_dir_grid"]:
            long_mask, short_mask = apply_threshold_pair(trade_prob, dir_prob, float(thr_trade), float(thr_dir))
            ...
            feasible = bt_metrics["n_trades"] >= int(cfg["min_validation_trades"]) and finite_or_default(coverage, 0.0) >= float(cfg["min_validation_coverage"])
            row = {"thr_trade": float(thr_trade), "thr_dir": float(thr_dir), "coverage": coverage, "feasible": bool(feasible), **bt_metrics}
            rows.append(row)

            key = threshold_selection_key(bt_metrics, feasible, coverage, cfg)
            if best_key is None or key > best_key:
                best_key = key
                best_pair = {"thr_trade": float(thr_trade), "thr_dir": float(thr_dir), "coverage": coverage, "feasible": bool(feasible)}
    ...
    best_long_mask, best_short_mask = apply_threshold_pair(trade_prob, dir_prob, float(best_pair["thr_trade"]), float(best_pair["thr_dir"]))
...
def evaluate_prediction_pack(
    ...
    soft_utility_vec = compute_soft_utility_numpy(trade_logit, dir_logit, y_ret, k=float(cfg["utility_tanh_k"]))
    ...
    selection_score = float(finite_or_default(scaled_soft_utility, 0.0) + 0.45 * finite_or_default(dir_auc, 0.5) + 0.15 * finite_or_default(trade_auc, 0.5))
    ...
    if search_threshold_pair_on_pack or selected_threshold_pair is None:
        ...
        long_mask, short_mask = apply_threshold_pair(trade_prob, dir_prob, float(selected_threshold_pair["thr_trade"]), float(selected_threshold_pair["thr_dir"]))
...
@torch.no_grad()
def predict_on_indices(model: nn.Module, x_node_scaled: np.ndarray, indices: np.ndarray, batch_size: int) -> Dict[str, Any]:
    ds = TemporalDataset(
    ...
    loader = DataLoader(ds, batch_size=int(batch_size), shuffle=False, num_workers=0, collate_fn=temporal_collate)

    model.eval()
    trade_logits: List[np.ndarray] = []
    dir_logits: List[np.ndarray] = []
    return_preds: List[np.ndarray] = []
    exit_type_logits: List[np.ndarray] = []
    tte_preds: List[np.ndarray] = []
    y_ret_all: List[np.ndarray] = []
    y_trade_all: List[np.ndarray] = []
    y_dir_all: List[np.ndarray] = []
    y_dir_mask_all: List[np.ndarray] = []
    y_exit_type_all: List[np.ndarray] = []
    y_tte_all: List[np.ndarray] = []
    sample_idx_all: List[np.ndarray] = []
    raw_t_all: List[np.ndarray] = []

    for batch in loader:
        x_node_seq = batch["x_node_seq"].to(DEVICE).float()
        outputs = model(x_node_seq, return_aux=False)
...
def train_one_split(
    ...
    train_loader = DataLoader(train_ds, batch_size=int(cfg["batch_size"]), shuffle=True, num_workers=0, collate_fn=temporal_collate)
    ...
    for epoch in range(1, int(cfg["epochs"]) + 1):
        ...
        train_trade_loss: List[float] = []
        train_dir_loss: List[float] = []
        train_ret_loss: List[float] = []
        train_exit_type_loss: List[float] = []
        train_tte_loss: List[float] = []
        train_utility_loss: List[float] = []
        train_false_positive_penalty: List[float] = []
        train_timeout_penalty: List[float] = []
        train_soft_utility: List[float] = []

        for batch in train_loader:
            x_node_seq = batch["x_node_seq"].to(DEVICE).float()
            target_batch = {key: value.to(DEVICE) for key, value in batch.items() if key not in {"x_node_seq", "sample_idx", "raw_t"}}

            optimizer.zero_grad(set_to_none=True)
            outputs = model(x_node_seq, return_aux=False)
            loss_pack = compute_total_loss(outputs=outputs, batch=target_batch, loss_state=loss_state, cfg=cfg)
        ...
        val_pred_pack = predict_on_indices(model=model, x_node_scaled=x_node_scaled, indices=idx_val, batch_size=int(cfg["batch_size"]))
        ...
        LOGGER.info(
            "[%s][%s] epoch=%02d loss=%.6f trade_bce=%.6f dir_bce=%.6f ret_huber=%.6f exit_ce=%.6f tte_huber=%.6f utility_loss=%.6f fp_pen=%.6f timeout_pen=%.6f train_soft_util=%.6f val_selection=%.6f val_soft_util=%.6f val_dir_auc=%.4f val_trade_auc=%.4f val_pnl_sum=%.6f val_ppt=%.6f val_trades=%s thr_trade=%.2f thr_dir=%.2f lr=%.2e epoch_sec=%.2f",
            split_name,
            cfg["graph_operator"],
            epoch,
            float(np.mean(train_total_loss)),
            float(np.mean(train_trade_loss)),
            float(np.mean(train_dir_loss)),
            float(np.mean(train_ret_loss)),
            float(np.mean(train_exit_type_loss)),
            float(np.mean(train_tte_loss)),
            float(np.mean(train_utility_loss)),
            float(np.mean(train_false_positive_penalty)),
            float(np.mean(train_timeout_penalty)),
            float(np.mean(train_soft_utility)),
        ...
        if better_selection_key(checkpoint_key, best_checkpoint_key):
            ...
            best_checkpoint_summary = {
                ...
                "val_metrics": {k: v for k, v in val_metrics.items() if k not in {"trades_df", "selected_threshold_pair"}},
    ...
    if evaluate_test_split:
        test_pred_pack = predict_on_indices(model=model, x_node_scaled=x_node_scaled, indices=idx_test, batch_size=int(cfg["batch_size"]))
    ...
    return SplitArtifacts(
        ...
        loss_state={"pos_weight_trade": float(loss_state.pos_weight_trade), "pos_weight_dir": float(loss_state.pos_weight_dir)},
...
def save_bundle(bundle_dir: Path, bundle_name: str, model_state: Dict[str, torch.Tensor], node_scaler_params: Dict[str, Any], cfg: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Path]:
...
@torch.no_grad()
def evaluate_saved_bundle_on_indices(bundle_dir: Path, bundle_name: str, indices: np.ndarray, label: str) -> Dict[str, Any]:
    ...
    pred_pack = predict_on_indices(model=model, x_node_scaled=x_node_scaled, indices=indices.astype(np.int64), batch_size=int(cfg["batch_size"]))
    ...
    return {"pred_pack": pred_pack, "metrics": metrics, "selected_threshold_pair": selected_threshold_pair}


def run_cv_for_operator(operator_name: str, base_cfg: Dict[str, Any], is_ablation_context: bool = True) -> Dict[str, Any]:
    run_cfg = copy.deepcopy(base_cfg)
    run_cfg["graph_operator"] = canonicalize_model_name(operator_name)
    ...
    for fold_idx, (idx_train, idx_val, idx_test) in enumerate(WALK_FORWARD_SPLITS, start=1):
        LOGGER.info("MODEL=%s | FOLD %s/%s train=%s val=%s test=%s", operator_name, fold_idx, len(WALK_FORWARD_SPLITS), len(idx_train), len(idx_val), len(idx_test))
        ...
        artifacts.validation_threshold_grid.to_csv(operator_dir / f"{bundle_name}_validation_threshold_grid.csv", index=False)
        artifacts.val_metrics["trades_df"].to_csv(operator_dir / f"{bundle_name}_val_trades.csv", index=False)
        artifacts.test_metrics["trades_df"].to_csv(operator_dir / f"{bundle_name}_test_trades.csv", index=False)
        ...
        if better_selection_key(artifacts.best_checkpoint_key, best_cv_key):
            best_cv_key = artifacts.best_checkpoint_key
...
def select_best_operator_from_cv_runs(operator_runs: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    operator_comparison_df = pd.DataFrame(rows).sort_values(
        by=["cv_mean_test_pnl_sum", "cv_mean_test_selection_score", "cv_mean_test_dir_auc", "cv_mean_test_pnl_per_trade"],
...
def build_final_holdout_trade_log_df(pred_pack: Dict[str, Any], trades_df: pd.DataFrame, selected_threshold_pair: Dict[str, Any], cfg: Dict[str, Any]) -> pd.DataFrame:
    trade_log_columns = [
        "trade_no",
        "entry_id",
        "exit_id",
    ...
    for trade_no, trade in enumerate(trades_df.to_dict(orient="records"), start=1):
        ...
        entry_sample_id = int(sample_idx_arr[entry_local_idx]) if 0 <= entry_local_idx < len(sample_idx_arr) else raw_t_to_sample_id.get(entry_raw_t)
        ...
        direction_margin = float(dir_prob - thr_dir) if side > 0 else float((1.0 - thr_dir) - dir_prob)
...
def run_selected_operator_post_cv_and_production(operator_run: Dict[str, Any], cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    production_artifacts.validation_threshold_grid.to_csv(operator_dir / f"{production_bundle_name}_validation_threshold_grid.csv", index=False)
    production_artifacts.val_metrics["trades_df"].to_csv(operator_dir / f"{production_bundle_name}_val_trades.csv", index=False)

    holdout_evaluations = [
        {"model_role": "best_cv_model", "bundle_name": best_cv_bundle_name, "label": f"FINAL BLIND HOLDOUT [{operator_name}] USING BEST CV MODEL"},
        {"model_role": "last_cv_fold_model", "bundle_name": last_cv_bundle_name, "label": f"FINAL BLIND HOLDOUT [{operator_name}] USING LAST CV FOLD MODEL"},
        {
            ...
            "best_epoch_cumulative_train_duration_sec": production_artifacts.timing["best_epoch_cumulative_train_duration_sec"],
        },
    ]
    ...
    for eval_spec in holdout_evaluations:
        ...
        for extra_key in ["fit_start_utc", "fit_end_utc", "fit_duration_sec", "best_epoch_cumulative_train_duration_sec"]:
            if extra_key in eval_spec:
                row[extra_key] = eval_spec[extra_key]
        holdout_rows.append(row)
        holdout_result["metrics"]["trades_df"].to_csv(operator_dir / f"{operator_name}_final_holdout_{eval_spec['model_role']}_trades.csv", index=False)
        if str(eval_spec["model_role"]) == "last_cv_fold_model":
            ...
            last_cv_trade_log_df.to_csv(operator_dir / f"{operator_name}_final_holdout_last_cv_trade_log.csv", index=False)

    final_holdout_comparison_df = pd.DataFrame(holdout_rows)
    final_holdout_comparison_df.to_csv(operator_dir / f"{operator_name}_final_holdout_model_comparison_summary.csv", index=False)
...
def build_artifact_manifest(root: Path) -> List[Dict[str, Any]]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_file():
            manifest.append({"relative_path": safe_relpath(path, root), "size_bytes": path.stat().st_size})
    return manifest
...
def normalize_holdout_model_state(model_role: Any) -> str:
    mapping = {"best_cv_model": "best_cv", "last_cv_fold_model": "last_cv", "final_refit_model": "final_refit"}
    return mapping.get(str(model_role), str(model_role))


def reorder_columns(df_: pd.DataFrame, preferred_columns: List[str]) -> pd.DataFrame:
    preferred = [column for column in preferred_columns if column in df_.columns]
    remainder = [column for column in df_.columns if column not in preferred]
    return df_.loc[:, preferred + remainder]
...
def render_dataframe_html(df_: pd.DataFrame, empty_message: str) -> str:
    if df_.empty:
        return f"<p>{html.escape(empty_message)}</p>"
    printable = df_.copy().where(pd.notnull(df_), "")
    return printable.to_html(index=False, border=0, escape=True, float_format=lambda value: f"{value:.6f}")


def build_final_report(artifact_root: Path, gcs_run_prefix: Optional[str] = None) -> Dict[str, Path]:
    resolved_config_path = find_single_file(artifact_root, "resolved_config.yaml") or find_single_file(artifact_root, "resolved_config.json")
    ...
    model_pipeline = str(resolved_config.get("model_pipeline") or CFG.get("model_pipeline") or "eth_baseline")
    ...
    for holdout_path in holdout_paths:
        ...
        if "best_epoch_cumulative_train_duration_sec" in display_df.columns:
            display_df["best_epoch_cumulative_train_duration_min"] = display_df["best_epoch_cumulative_train_duration_sec"].apply(seconds_to_rounded_minutes)
        ...
        if not final_refit_row.empty:
            ...
            final_refit_best_epoch_minutes = seconds_to_rounded_minutes(final_refit_row.iloc[0].get("best_epoch_cumulative_train_duration_sec"))
    ...
    run_timing_pairs = [
        ...
        ("Total runtime", format_minutes_label(env_meta.get("total_runtime_seconds") or run_summary.get("total_runtime_seconds"))),
    ]

    operator_sections_html = "".join(
        f"<h2>Final holdout - {html.escape(operator_name)}</h2>{render_dataframe_html(display_df, f'No final_holdout metrics found for {operator_name}.')}"
        for operator_name, display_df in operator_sections
    )
    environment_pairs = [(key, value) for key, value in env_meta.items() if isinstance(value, (str, int, float, bool)) or value is None]
...
def expected_required_files(cfg: Dict[str, Any]) -> List[str]:
    freq = normalize_freq_name(cfg["freq"])
    return [f"{cfg['target_asset']}_{freq}.csv"]
...
def build_success_email_attachments(artifact_root: Path, report_paths: Dict[str, Path], resolved_config_path: Optional[Path]) -> List[Path]:
    candidates = [
