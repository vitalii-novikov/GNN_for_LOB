...
![Figure 1.1 - Conceptual pipeline](figures_generated/fig_1_1_ChatGPT_Image.png)

Figure 1.1 presents the controlled research pipeline from frequency-specific ADA, BTC, and ETH LOB snapshots through graph-based entry decisions and cost-aware final-holdout evaluation.
...
![Figure 1.2 - Research question map](figures_generated/fig_1_2_research_question_map.png)
...
![Figure 3.1 - Graph input representation](figures_generated/fig_3_1_graph_input_representation.png)

Figure 3.1 shows the three-asset graph input representation with ADA, BTC, and ETH nodes, ETH as the target asset, and relation-aware edge channels for `price_dep`, `order_flow`, and `liquidity`.
...
![Figure 3.2 - Split design](figures_generated/fig_3_2_split_design_v2.png)

Figure 3.2 compares frequency-specific working samples, pre-holdout development regions, and aligned final-holdout intervals for the 5min, 1min, and frequency-adapted 1sec regimes.
...
![Figure 3.3 - Triple barrier](figures_generated/fig_3_3_triple_barrier_eth_midpoint.png)

Figure 3.3 illustrates ETH triple-barrier target construction, including the target timestamp, volatility-scaled barriers, vertical barrier, realized event exit, realized return, trade relevance label, and direction label.
...
![Figure 3.4 - Entry backtest and PnL](figures_generated/fig_3_4_entry_backtest_post_cost_pnl.png)

Figure 3.4 summarizes the common entry-model backtest: the trade head activates a candidate, the direction head selects long or short exposure, the realized event rule closes the position, and the shared transaction-cost proxy converts gross PnL to net PnL.
...
![Figure 3.5 - Walk-forward validation](figures_generated/fig_3_5_walkforward_model_states.png)

Figure 3.5 shows purged walk-forward validation with chronological train, purge, validation, purge, and test segments, followed by the protected final holdout and the derived `best_CV`, `last_CV`, and `final_refit` states.
...
![Figure 3.6 - Metric hierarchy](figures_generated/fig_3_6_metric_hierarchy.png)

Figure 3.6 separates predictive ranking diagnostics, pre-cost signal extraction, selectivity and turnover evidence, and the primary post-cost economic outcome `pnl_sum`.
...
![Figure 4.1 - Architecture comparison](figures_generated/fig_4_1_architecture_comparison.png)

Figure 4.1 compares `base_gnn`, `multigraph`, and `memorygraph`, highlighting early relation fusion, relation-specific graph pathways, recurrent node-edge memory, and the shared multi-task output heads.
...
Architecturally, `base_gnn` tests whether the three relation channels can be compressed into a single edge representation before graph message passing without losing economically useful information.
...
```text
base_gnn forward path:
X_node -> NodeTemporalEncoder
X_edge -> EdgeTemporalEncoder
(node_seq, edge_seq) -> HybridEdgeFeatureFusion
relation edge states -> EdgeRelationFusion
fused edges + node states -> SingleGraphOperatorBlock
node states -> target/global GraphReadout
readout sequence -> TargetTemporalTrunk
shared state -> trade, direction, return, exit-type, time-to-event heads
```
...
X_edge -> EdgeTemporalEncoder
(node_seq, edge_seq) -> HybridEdgeFeatureFusion
price_dep edges -> RelationGraphBlock(price_dep)
order_flow edges -> RelationGraphBlock(order_flow)
liquidity edges -> RelationGraphBlock(liquidity)
relation node states -> RelationAttentionFusion
fused node states -> target/global GraphReadout
readout sequence -> TargetTemporalTrunk
shared state -> trade, direction, return, exit-type, time-to-event heads
```
...
X_node_t -> NodeStepProjector
X_edge_t -> EdgeStepProjector
(node_t, edge_t) -> HybridEdgeFeatureFusion
edge_t + source/destination node context + previous edge_memory -> EdgeMemoryUpdater
edge-enriched state -> AdaptiveGraphConnectivity + graph operator
relation node states + relation edge-memory context + previous node_memory -> NodeMemoryUpdater
fused node sequence -> target/global GraphReadout
readout state -> output projection
shared state -> trade, direction, return, exit-type, time-to-event heads
```
...
![Figure 5.1 - Benchmark overview](figures_generated/fig_5_1_benchmark_overview.png)

Figure 5.1 visualizes `last_CV` net PnL (`pnl_sum`) across all eighteen model-frequency configurations, grouped by temporal resolution, graph family, and graph operator. The heatmap is descriptive and does not report uncertainty intervals or formal significance tests.
...
![Figure 5.4 - Gross vs net PnL](figures_generated/fig_5_4_gross_vs_net_1sec.png)

Figure 5.4 reports the same one-second result in static cross-sectional form. The upper panel compares gross and net PnL for the six one-second models, while the lower panel shows implied transaction-cost drag and trade count.
...
![Figure 5.6 - Model states comparison](figures_generated/fig_5_6_last_cv_vs_final_refit.png)

Figure 5.6 compares selected `last_CV` and `final_refit` cases on net PnL and ranking diagnostics. It shows why `final_refit` is informative but not a replacement for the chronological `last_CV` deployment reference.
...
Figure 6.1 summarizes the interpretation chain from predictive ranking through gross signal, turnover, costs, and model-state stability to deployment-informative post-cost evidence.
...
Figure 7.1 organizes future work into turnover-aware learning, execution-aware evaluation, larger graph universes, selective memory mechanisms, regime robustness, uncertainty quantification, and cost-sensitivity analysis.
...
## **Appendix D. Additional Figure-Based Robustness Diagnostics**

This appendix reports supplementary visual diagnostics that support the main interpretation without changing the primary ranking by `last_CV` `pnl_sum`. The figures focus on cost sensitivity and the relationship between signal extraction, turnover, and post-cost outcomes.

**Appendix Figure D.1. Cost-sensitivity curves for selected final-holdout models.**
![Appendix Figure D.1 - Cost sensitivity](figures_generated/appendix_fig_D_1_cost_sensitivity_curves.png)

Appendix Figure D.1 varies the assumed per-side transaction cost while holding each model's final-holdout gross PnL and trade count fixed. The benchmark cost assumption is marked at 1.0 bps per side. The curves show that high-turnover one-second models become economically infeasible under much lower cost assumptions than the lower-turnover shared-task winners.

**Appendix Figure D.2. Turnover, gross signal, and post-cost net PnL across last_CV configurations.**
![Appendix Figure D.2 - Turnover scatter](figures_generated/appendix_fig_D_2_turnover_scatter.png)

Appendix Figure D.2 visualizes the relationship between trade activity, gross signal, and net PnL across all eighteen `last_CV` configurations. The figure supports the main conclusion that higher gross PnL does not necessarily imply higher net PnL when turnover and transaction costs increase.
