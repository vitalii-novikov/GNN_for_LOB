...
def resolve_runtime_overrides(cfg: Dict[str, Any], config_path: Path) -> Dict[str, Any]:
    ...
    artifact_root = artifact_base_root / run_id

    resolved["data_dir"] = str(data_dir)
    resolved["artifact_root_base"] = str(artifact_base_root)
...
def download_from_gcs(required_files: List[str], gcs_data_prefix: str, local_data_dir: Path) -> List[Path]:
    ...
    Args:
        required_files: Expected CSV filenames, e.g. ["BTC_1min.csv"].
        gcs_data_prefix: Prefix like gs://bucket/path/to/data.
        local_data_dir: Local directory where files should exist.

    Returns:
        List of local file paths corresponding to required_files.

    Raises:
        FileNotFoundError: If a required object is absent in GCS after resolution.
        Exception: If a download attempt fails.
    """
...
def upload_artifacts_to_gcs(local_artifact_root: Path, gcs_run_prefix: str) -> List[str]:
    """Recursively upload local artifacts to GCS while preserving directory structure.

    Args:
        local_artifact_root: Local run artifact directory.
        gcs_run_prefix: Destination prefix like gs://bucket/runs/<run_id>.

    Returns:
        List of uploaded gs:// object URIs.

    Raises:
        ValueError: If gcs_run_prefix is empty.
...
def main() -> None:
    ...
    add_cli_override_args(parser)
    args = parser.parse_args()

    config_path = Path(args.config).expanduser().resolve()

    run_start_dt = utc_now()
    ...
    try:
        initialize_runtime_globals(cfg)

        environment_metadata = collect_environment_metadata(
            run_id=run_id,
            run_start_time_utc=run_start_utc,
        )
        environment_metadata_path = ARTIFACT_ROOT / "environment_metadata.json"
        save_json(environment_metadata_path, environment_metadata)
        ...
        if bool(CFG["run_full_operator_ablation"]):
            ...
            for operator_name in CFG["operator_candidates"]:
                LOGGER.info("RUNNING CV FOR OPERATOR: %s", operator_name)
        ...
        if not args.skip_gcs_upload and str(CFG.get("gcs_run_prefix") or "").strip():
            ...
            environment_metadata = finalize_environment_metadata(
            ...
            write_run_summary(ARTIFACT_ROOT, run_summary)
            ...
            if gcs_run_prefix:
                ...
                gcs_summary_path = ARTIFACT_ROOT / "gcs_upload_summary.json"
        ...
        if not args.skip_email:
            subject = f"[SUCCESS] run_id={RUN_ID} operator={selected_operator}"
            html_body = (
                report_paths["html"].read_text(encoding="utf-8")
                if report_paths
                else "<p>Training completed.</p>"
            )
            attachments = build_success_email_attachments(
            ...
            send_email_report(
                subject=subject,
                html_body=html_body,
                to_email=str(CFG.get("email_to") or ""),
                attachments=attachments,
                smtp_settings=get_smtp_settings(CFG),
            )

        LOGGER.info("Notebook build complete.")
        ...
        failure_traceback_path = ARTIFACT_ROOT / "failure_traceback.txt"
        ...
        try:
            ...
            LOGGER.exception("Failed while handling top-level exception.")
